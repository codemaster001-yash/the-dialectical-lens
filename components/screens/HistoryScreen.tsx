...
const generatePdfDocument = (session: DebateSession) => {
  ...
  doc
    ...
    .setFontSize(22)
    .setTextColor(accentColor[0], accentColor[1], accentColor[2]);
  ...
  doc
    ...
    .setFontSize(9)
    .setTextColor(subtleTextColor[0], subtleTextColor[1], subtleTextColor[2]);
  checkPageBreak(5);
  doc.text(
  ...
  if (session.conclusion) {
    addSection("Conclusion & Analysis", () => {
      ...
      addSubSection("Key Agreements", () =>
        addList(session.conclusion!.agreement_points)
      );
      addSubSection("Key Conflicts", () =>
        addList(session.conclusion!.conflict_points)
      );
      addSubSection("Bridging Questions", () =>
        addList(session.conclusion!.bridging_questions)
      );
    });
  }
...
}> = ({ session, onBack, onReplay, onDownload }) => {
  return (
    <div className="bg-light-secondary dark:bg-dark-secondary p-8 rounded-2xl shadow-lg">
      ...
      <div className="bg-light-primary dark:bg-dark-primary p-6 rounded-xl">
        <div className="mb-6">
          <h3 className="text-lg font-bold text-light-text/90 dark:text-dark-text/90">
            Full Topic
          </h3>
        ...
        <h3 className="text-xl font-bold mb-4 border-b border-gray-300 dark:border-gray-600 pb-2">
          Personas
        </h3>
        <div className="space-y-6 mb-6">
          {session.personas.map((p) => (
            <div
              key={p.name}
              className="bg-light-secondary dark:bg-dark-secondary p-4 rounded-xl shadow-sm"
            >
              <div className="mb-3">
                <p className="font-bold text-lg text-light-accent dark:text-dark-accent">
                  {p.name}
                </p>
                <p className="font-semibold text-md text-light-text/90 dark:text-dark-text/90">
                  {p.title}
                </p>
              </div>
              ...
              <div className="text-sm space-y-3">
                <div>
                  <h4 className="font-bold text-light-text/80 dark:text-dark-text/80">
        ...
        <div className="space-y-4 max-h-[40vh] overflow-y-auto p-4 bg-light-secondary dark:bg-dark-secondary rounded-xl">
          {session.chatLog.map((chat, i) => (
            <div key={i} className="text-sm">
              <span className="font-bold">{chat.personaName}:</span>{" "}
              {chat.message}
            </div>
          ))}
        </div>
          ...
          <div className="mt-6 border-t border-gray-300 dark:border-gray-600 pt-4">
            ...
            <div className="grid md:grid-cols-2 gap-6 text-sm">
              <div>
                ...
                <ul className="list-disc list-inside space-y-1 text-light-text/80 dark:text-dark-text/80">
                  {session.conclusion.agreement_points.map((p, i) => (
                    <li key={i}>{p}</li>
                  ))}
                </ul>
              </div>
              <div>
                ...
                <ul className="list-disc list-inside space-y-1 text-light-text/80 dark:text-dark-text/80">
                  {session.conclusion.conflict_points.map((p, i) => (
                    <li key={i}>{p}</li>
                  ))}
                </ul>
              </div>
              <div className="md:col-span-2">
                ...
                <ul className="list-disc list-inside space-y-1 text-light-text/80 dark:text-dark-text/80">
                  {session.conclusion.bridging_questions.map((p, i) => (
                    <li key={i}>{p}</li>
                  ))}
                </ul>
              </div>
              ...
              {session.conclusion.action_items &&
                session.conclusion.action_items.length > 0 && (
                  <div className="md:col-span-2 border-t border-gray-300 dark:border-gray-600 pt-4 mt-2">
                    <h4 className="font-bold mb-2">Actionable Suggestions</h4>
                    <div className="space-y-3">
                      {session.conclusion.action_items.map((item, index) => (
                        <div key={index}>
                          <p className="font-semibold text-light-text/90 dark:text-dark-text/90">
                            {item.personaName}:
                          </p>
      ...
      <div className="text-center mt-8 flex justify-center items-center gap-4">
        <button
          ...
          className="px-6 py-3 bg-light-accent text-white font-bold rounded-lg shadow-md hover:bg-indigo-700 dark:hover:bg-indigo-500 transition-colors flex items-center justify-center gap-2"
        >
          <DownloadIcon /> Download PDF
        </button>
        <button
          onClick={() => onReplay(session)}
          className="px-6 py-3 bg-gray-600 text-white font-bold rounded-lg shadow-md hover:bg-gray-700 transition-colors flex items-center justify-center gap-2"
        >
...
const HistoryScreen: React.FC<{
  ...
  const [selectedSession, setSelectedSession] = useState<DebateSession | null>(
    null
  );
      ...
      <div className="bg-light-secondary dark:bg-dark-secondary p-8 rounded-2xl shadow-lg">
        ...
        <div className="flex flex-col md:flex-row gap-4 mb-6">
          <input
            ...
            className="w-full bg-light-primary dark:bg-dark-primary border border-gray-300 dark:border-gray-600 rounded-lg py-2 px-3 focus:outline-none focus:ring-light-accent"
          />
          <select
            value={sortOrder}
            onChange={(e) => setSortOrder(e.target.value as any)}
            className="bg-light-primary dark:bg-dark-primary border border-gray-300 dark:border-gray-600 rounded-lg py-2 px-3 focus:outline-none focus:ring-light-accent"
          >
        ...
        {loading ? (
          <div className="flex justify-center p-8">
            <Spinner className="w-10 h-10 text-light-accent dark:text-dark-accent" />
          </div>
        ) : (
          <div className="space-y-4">
            {filteredSessions.length > 0 ? (
              filteredSessions.map((session) => (
                <div
                  key={session.id}
                  className="bg-light-primary dark:bg-dark-primary p-4 rounded-lg flex items-center justify-between hover:shadow-md transition-shadow"
                >
                  <div
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    <button
                      title="Replay"
                      onClick={() => onReplay(session)}
                      className="p-2 text-light-accent dark:text-dark-accent hover:bg-black/10 dark:hover:bg-white/10 rounded-full transition-colors"
                    >
                      <ReplayIcon />
                    </button>
                    <button
              ...
              <p className="text-center text-gray-500 py-8">
                No sessions found. Start a new convolution to see it here.
              </p>
            )}
          </div>
        )}
