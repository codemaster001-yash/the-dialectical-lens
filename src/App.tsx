...
const App: React.FC = () => {
  ...
  const [sessionToReplay, setSessionToReplay] = useState<DebateSession | null>(
    null
  );
  ...
  const handleSetupComplete = async (
    ...
    try {
      ...
      setCurrentSession({
        topic,
        title,
        personas: participants.map((p) => ({ userInput: p } as Persona)),
      });
  ...
  const handlePersonaUpdate = (participantInputs: PersonaInput[]) => {
    setCurrentSession((prev) => ({
      ...prev,
      personas: participantInputs.map((p) => ({ userInput: p } as Persona)),
    }));
  };
  ...
  const renderScreen = () => {
    switch (screen) {
      ...
      case Screen.PersonaGallery:
        return (
          <PersonaGalleryScreen
            personas={currentSession.personas as Persona[]}
      ...
      case Screen.Replay:
        ...
        return (
          <ReplayScreen session={sessionToReplay} onBack={handleViewHistory} />
        );
      default:
    ...
    <div className="min-h-screen w-full transition-colors duration-300">
      <header className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between p-4 bg-light-primary/80 dark:bg-dark-primary/80 backdrop-blur-sm">
        ...
        >
          ...
          >
            <path d="M50,10A40,40,0,0,0,14.6,25.4L30,40.8A20,20,0,1,1,30,60l-15.4,15.4A40,40,0,1,0,50,10Z" />
          </svg>
      ...
      <main className="pt-24 pb-12 px-4 sm:px-6 lg:px-8">
        {error && (
          <div
            className="fixed top-20 left-1/2 -translate-x-1/2 z-50 bg-red-500 text-white py-2 px-4 rounded-md shadow-lg animate-fade-in"
