...
import { runDebateTurnStream, synthesizeConclusion } from '../../services/geminiService';
import { addSession } from '../../services/dbService';
import type { DebateSession, Persona, ChatMessage, Conclusion } from '../../types';
...
const Countdown: React.FC<{ onEnd: () => void }> = ({ onEnd }) => {
    ...
    return (
        <div className="absolute inset-0 bg-light-secondary/80 dark:bg-dark-secondary/80 flex items-center justify-center z-20 backdrop-blur-sm">
            <div className="text-8xl font-bold text-light-accent dark:text-dark-accent animate-ping duration-1000">{count}</div>
        </div>
    )
}

const ConclusionModal: React.FC<{ conclusion: Conclusion, onClose: () => void }> = ({ conclusion, onClose }) => (
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 animate-fade-in">
        <div className="bg-light-secondary dark:bg-dark-secondary rounded-2xl shadow-xl max-w-3xl w-full max-h-[90vh] flex flex-col">
            <h2 className="text-2xl font-bold p-6 text-center border-b border-gray-200 dark:border-gray-700 flex-shrink-0">Resolution Summary</h2>
            <div className="p-6 overflow-y-auto space-y-6">
                <div>
                    <h3 className="font-bold text-lg mb-2 text-light-accent dark:text-dark-accent">Final Conclusion</h3>
                    <p className="text-light-text/80 dark:text-dark-text/80">{conclusion.conclusion}</p>
                </div>
                {conclusion.action_items && conclusion.action_items.length > 0 && (
                     <div>
                        <h3 className="font-bold text-lg mb-3 text-light-accent dark:text-dark-accent">Actionable Suggestions</h3>
                        <div className="space-y-4">
                            {conclusion.action_items.map((item, index) => (
                                <div key={index} className="p-3 bg-light-primary dark:bg-dark-primary rounded-lg">
                                    <p className="font-semibold text-light-text dark:text-dark-text">{item.personaName}:</p>
            ...
            <div className="p-4 mt-auto border-t border-gray-200 dark:border-gray-700 text-center flex-shrink-0">
                <button onClick={onClose} className="px-6 py-3 bg-success text-white font-bold rounded-lg shadow-md hover:bg-success-hover transition-all duration-300">
                    Close & View History
                </button>
            </div>
        </div>
...
const DebateScreen: React.FC<DebateScreenProps> = ({ session, onDebateComplete, onError }) => {
  ...
  const concludeDebate = useCallback(async () => {
    ...
    try {
        ...
        if (finalChatLog.length === 0) {
            onDebateComplete({ ...session, id: new Date().toISOString(), createdAt: new Date().toISOString(), chatLog: [], conclusion: null });
            return;
        }

        const conclusionResult = await synthesizeConclusion(session.topic, finalChatLog);
  ...
  useEffect(() => {
    ...
    const runTurn = async () => {
        ...
        try {
            const turnGenerator = runDebateTurnStream(session.topic, session.personas, chatLogRef.current);
  ...
  return (
    <div className="max-w-4xl mx-auto h-[75vh] flex flex-col bg-light-secondary dark:bg-dark-secondary rounded-2xl shadow-2xl">
      ...
      {isComplete && conclusion && finalSession && (
        <ConclusionModal conclusion={conclusion} onClose={() => onDebateComplete(finalSession)} />
      )}
      ...
      <div className="p-4 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center flex-wrap gap-y-2">
        ...
        <div className="flex items-center justify-end gap-4 flex-1">
             {isDebating && !isSynthesizing && (
                <button onClick={handleConcludeClick} className="px-4 py-2 text-sm bg-light-accent text-white font-bold rounded-lg shadow-md hover:bg-indigo-700 dark:hover:bg-indigo-500 transition-all duration-300 disabled:opacity-50">
                    Conclude Debate
                </button>
            )}
      ...
      <div className="flex-grow p-6 space-y-6 overflow-y-auto">
        {chatLog.map((chat, index) => {
            ...
            const indexValue: number = personaIndex === undefined ? 0 : personaIndex;
            const isSelf = indexValue % 2 === 0;

            return (
                <div key={`${chat.timestamp}-${index}`} className={`flex items-end gap-3 ${isSelf ? 'justify-end' : ''}`}>
                    ...
                    <div className={`max-w-md p-3 rounded-lg ${isSelf ? 'bg-light-accent text-white rounded-br-none' : 'bg-gray-200 dark:bg-gray-700 rounded-bl-none'}`}>
