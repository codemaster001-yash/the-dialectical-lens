...
const getAiClient = (): GoogleGenAI => {
  ...
  if (!isApiKeyConfigured()) {
    throw new Error(
      "API key is not configured. Please check your .env file or Netlify environment variables."
    );
  }
...
export const getWelcomeMessage = async (): Promise<string> => {
  try {
    ...
    const prompt =
      "Translate the word 'Welcome' into a random, non-English language. Provide only the single, translated word as a plain string, without any additional text, formatting, or quotation marks.";
...
export const generatePersona = async (
  ...
  const client = getAiClient();
  const prompt = `You are a creative writer specializing in character development. Based on the following user-provided details, create a rich, empathetic, and detailed expert persona. User Details: ${JSON.stringify(
    input
  ...
  if (!responseText) {
    throw new Error(
      "Received an empty response from the AI when generating a persona."
    );
  }
...
Your task is to provide a final analysis as a JSON object matching the provided schema. This includes summarizing agreements, conflicts, and proposing bridging questions. Crucially, you must also provide an 'action_items' array. For each participant, provide a 'personaName' and a 'suggestions' array containing exactly two concrete, actionable next steps they could take to help resolve the conflict.`;
  ...
  if (!responseText) {
    throw new Error(
      "Received an empty response from the AI when synthesizing the conclusion."
    );
  }
